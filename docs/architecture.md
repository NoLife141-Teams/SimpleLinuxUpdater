...
- `internal/servers.Service` owns server CRUD, tag normalization, secret persistence, rollback behavior, and per-server known-host operations. Server names remain the persistent identity; names are unique case-insensitively, while SSH endpoints are unique by normalized DNS/IP host plus normalized port.
- `internal/updates.Service` owns update, baseline and plan-aware pre-checks, the explicit non-interactive APT/dpkg policy, APT reconciliation/repair, controlled reboot and verification, autoremove, sudoers, approval, scheduled-scan, job, and audit runner behavior; it consumes Host Maintenance Session for authenticated host execution. Current non-root privileged maintenance crosses a root-owned helper boundary with typed operations and strict package-selector validation rather than granting APT executables directly in sudoers; only the exact pre-helper `fuser` lock probe remains as a transitional fallback.
...
One runner owns one session at a time. CVE enrichment opens an independent read-only session, collects distribution and source-package identities, links each selected APT package index to its local `InRelease` metadata, verifies that metadata with the Debian or Ubuntu archive keyring, and evaluates installed and candidate versions through cached OSV queries. Confirmed installed-version findings are separated into vulnerabilities fixed by the candidate and vulnerabilities that remain affected. An official candidate remains scannable when APT no longer exposes the installed version's origin, but the result is labelled as having unverified installed provenance; explicit third-party, unsigned, or unsupported origins retain unknown coverage. A manual approval wait keeps the Server Action reserved but closes the discovery session; approval opens a fresh session before the approved upgrade, while cancellation or approval timeout does not reconnect.
