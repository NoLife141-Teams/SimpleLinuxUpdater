...
The recommended way to install the restricted rule is **Enable apt** on the Status page. It writes and validates the exact command set expected by the running version, including:

- read-only and mutating `apt-get` operations;
- the explicit `/usr/bin/env` non-interactive APT and dpkg command forms;
- `dpkg --audit` and the restricted `dpkg --configure -a` repair form;
- both APT/DPKG `fuser` lock probes;
- the exact `/usr/bin/systemctl reboot` command used by controlled reboot.

Re-run **Enable apt** after upgrading from a version that predates APT repair, explicit non-interactive commands, or controlled reboot. If you maintain `/etc/sudoers.d/apt-nopasswd` manually, compare it with the rule generated by the running release and validate it with:
