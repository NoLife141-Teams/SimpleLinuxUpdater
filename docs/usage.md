...
After `apt-get update`, package discovery uses APT simulation plus read-only `--print-uris --download-only` planning so no archive is fetched or package changed. A second disk-space gate then evaluates the actual upgrade plan before approval. When APT reports both archive-download size and installed-size delta, the gate reserves the archive bytes, positive installed growth, a bounded 10% safety margin (`64 MiB` minimum, `512 MiB` maximum), and the existing `1 GiB` operational reserve. Freed installed space contributes zero required growth. The archive cache, `/var`, and `/` are checked by filesystem device so shared filesystems are deduplicated and split filesystems are protected independently.
