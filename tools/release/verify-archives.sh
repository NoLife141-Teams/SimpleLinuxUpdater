...
(
  cd "$root/$app"
  exec env DEBIAN_UPDATER_LISTEN_ADDR=127.0.0.1:18081 DEBIAN_UPDATER_DB_PATH="$root/servers.db" ./webserver >/dev/null 2>&1
) &
pid=$!
for ((attempt=0; attempt<60; attempt++)); do
  if curl -fsSL http://127.0.0.1:18081/login >/dev/null; then break; fi
  kill -0 "$pid"
  sleep 1
done
curl -fsSL http://127.0.0.1:18081/login >/dev/null
